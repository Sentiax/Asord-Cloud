...
<head>
	...
	<title>AsordCloud | Register</title>
...
<body class="bg-neutral-800">
	<div class="p-5 mx-auto mt-16 max-w-md rounded shadow-sm">
		...
		<input class="px-4 w-full h-12 rounded border border-none focus:outline-none bg-stone-900 text-neutral-300" name="email" placeholder="Email"
					type="email" required />
			<div>
			<input class="px-4 w-full h-12 rounded border border-none focus:outline-none bg-stone-900 text-neutral-300" name="name" placeholder="Username"
				type="text" minlength="${config.auth.min_name_length}" maxlength="${config.auth.max_name_length}" required /><br>
...
Swal.fire(
	'Email or username or password is not Found',
	'',
	'error'
  )
</html>
...
<body class="bg-neutral-800">
	<div class="p-5 mx-auto mt-16 max-w-md rounded shadow-sm">
		...
		} else if (!config.auth.require_register_key || !err) {
			res.send(`<!DOCTYPE html>
<html lang="en">
